...
_.safewrap = function (f) {
    return function () {
        try {
            ...
            console$1.critical('Implementation error. Please contact support@DATracker.com.');
        }
    };
};
...
_.info = {
    ...
    properties: function () {
        ...
        return _.extend(_.strip_empty_properties({
            ...
            'referrer': decodeURIComponent(document$1.referrer), // 说明：如果url上带有 @ 空格之类的，浏览器会做编码，这样子采集的url 不利于后面数据分析，故需要解析下
...
var pagetime = {
  ...
  customizeSetInterval: function (callback, interval) {
    ...
    if (!requestAnimationFrame) {
      console$1.error('定时上报数据：浏览器需支持 requestAnimationFrame  方法！');
      return;
    }
      ...
      if (currentUrl) {
        ...
        _.each(this.data.duration, function (item) {
          ...
          if (item.findType === 'querySelector') {
            divid += ',' + item.divid;
          } else if (item.findType === 'dataAttribute') {
  ...
  monitor: function () {
    ...
    if (!MutationObserver) {
      console$1.error('监听元素停留时长：浏览器需支持 MutationObserver 方法！');
      return;
    }
    if (!IntersectionObserver) {
      console$1.error('监听元素停留时长：浏览器需支持 IntersectionObserver 方法！');
      return;
    }
    var self = this;
            ...
            _.each(self.data.duration, function (item) {
              if (item.divid === divid) {
                ...
                if (entries[i].intersectionRatio <= self.config.el_threshold) {
                  ...
                  item.startTime = 0;
                  item.leave = true;
                  console$1.log(divid, '离开了');
                } else {
                  console$1.log(divid, '进来了');
                  item.leave = false;
                  // 进入了
                  if (item.startTime > 0) {
                    item.divtime += nowTime - item.startTime;
  ...
  mutantionFn: function () {
    var self = this;
    if (!self.intersectionObserver) {
      console$1.error('没有初始化 intersectionObserver 监听器');
      return;
    }
    _.each(self.data.duration, function (item) {
      try {
...
var heatmap = {
  ...
  start: function (ev, target, noPath, parentIsA) {
    ...
    if (ev) {
      ...
      prop.stat = prop.path + ',' + (mousePosi.left - domPosi.left) + ',' + (mousePosi.top - domPosi.top) + ',' + this.getDomWidth(target) + ',' + this.getDomHeight(target);
    }
  ...
  initHeatmap: function () {
    ...
    _.addEvent(document, 'click', function (e) {
      ...
      } else if (['button', 'input', 'textarea'].indexOf(tagName) > -1) {
  ...
  prepare: function (todo) {
    ...
    function isReady(data) {
      if (heatmap_url) {
        ...
        console$1.error('没有指定heatmap_url的路径');
      }
    }
...
var debug = {
    ...
    _loadControlJs: function () {
        ...
        if (control_js_url) {
            ...
            console$1.error('没有指定control_js_url的路径');
        }
    },
...
var visualizationAbest = {
    ...
    _setType: function () {
        console$1.log(this._visualizationArr);
        try {
...
DATrackerLib.prototype.init = function (token, config, name) {
    ...
    if (name === PRIMARY_INSTANCE_NAME) {
        console$1.error('You must initialize the main DATracker object right after you include the DATracker js snippet');
        return;
    }
...
DATrackerLib.prototype.track = function (event_name, properties, callback, setData_type, method) {
    ...
    if (_.isUndefined(event_name)) {
        console$1.error('No event name provided to DATracker.track');
        return;
    }
    ...
    console$1.log(PRIMARY_INSTANCE_NAME + ' REQUEST:');
    ...
    var jsTrack = function () {
        ...
        self._send_request(ajaxurl, { 'data': encoded_data, 'appKey': appkey_data }, self._prepare_callback(callback, truncated_data), errorFn, otherFn, nowMethod);
    };
    if (this.get_config('use_app_track')) {
...
DATrackerLib.prototype.time_event = function (event_name) {
    if (_.isUndefined(event_name)) {
        console$1.error('No event name provided to DATracker.time_event');
        return;
    }
...
DATrackerPeople.prototype._send_request = function (data, callback) {
    ...
    var jsTrack = function () {
        ...
        self._DATracker._send_request(url, { 'data': encoded_data, 'appKey': appkey_data }, self._DATracker._prepare_callback(callback, truncated_data));
    };
