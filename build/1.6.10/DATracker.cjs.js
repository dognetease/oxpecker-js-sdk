...
var heatmap = {
   ...
   */
  start: function (ev, target, noPath) {
    ...
    if (ev) {
      ...
      let mousePosi = this.getMousePosion(ev); // 鼠标相对页面的位置
      let domPosi = this.getDomPosition(target); // 元素相对页面的位置
    ...
    if (target.tagName.toLowerCase() === 'a' && heatmapConfig && heatmapConfig.isTrackLink === true) {
  ...
  initHeatmap: function () {
    ...
    _.addEvent(document, 'click', function (e) {
      ...
      var isA = that.hasParentA(target);
      if (isA || ['button', 'input', 'textarea'].indexOf(tagName) > -1) {
