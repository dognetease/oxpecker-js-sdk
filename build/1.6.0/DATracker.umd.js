(function (global, factory) {
  ...
  var abtest = {
      ...
      _default_variables: {},
      //保存已经使用的实验变量，用来判断指定的实验变量是否需要发送内置事件。
      //如果已经在里面的，就不用再发内置事件了。
      usedvariables: [],
      // 获取配置是否完成的标志
      getVariableInfoComplete: false,
      queue: [],
      _init: function (instance) {
          ...
          try {
              var abtest = this.DATracker.get_config('abtest');
              // 保存用户在sdk中配置的变量
              this._default_variables = abtest.default_variables || {};
              this.abtest_config = _.deepMerge(this.abtest_config, abtest);
              //判断是否启动ab测试
              if (this.abtest_config.enable_abtest) {
                  // 从预配置中获取默认变量
                  this.data.variables = this.abtest_config.default_variables;
                  // 到期了重新拉取配置
                  if (this._checkUpdateTime()) {
                      this.async_get_variable();
                  } else {
                      var localObj = JSON.parse(_.localStorage.get(this.localStorageName));
                      this.data = localObj.data;
                      this.usedvariables = localObj.usedvariables;
                      // 此时数据使用本地缓存，自然该标志设置为true
                      this.getVariableInfoComplete = true;
                  }
              }
          } catch (error) {
              this.async_get_variable();
      ...
      // 保存已使用的实验变量
      _saveUsedvariables: function (variable) {
          if (!_.include(this.usedvariables, variable)) {
      ...
      //当前平台不支持多变量
      _track_abtest: function (attributes) {
          if (!attributes) return;
          this.DATracker.track('da_abtest', attributes);
      },
      // 外部回调函数中调用
      get: function (variable, defalut) {
          try {
              if (typeof this.data.variables[variable] !== 'undefined') {
                  if (!_.include(this.usedvariables, variable)) {
                      //TODO: 多变量如何处理，对应的实验问题
                      var attributes = {};
                      var sendOtherObj = this._variableToFindExperiment(variable);
                      attributes[variable] = this.data.variables[variable];
                      for (var key in sendOtherObj) {
                          if (sendOtherObj.hasOwnProperty(key)) {
                              attributes[key] = sendOtherObj[key];
                          }
                      }
                      // 如果该变量没有 experimentId 或者 versionId
                      if (attributes.experimentId !== "" && attributes.versionId !== "") {
                          this._track_abtest(attributes);
                          this._saveUsedvariables(variable);
                      }
                  }
      ...
      get_variation: function (callback) {
          if (!this.getVariableInfoComplete) {
              return this.queue.push(arguments), !1;
          }
      ...
      async_get_variable: function () {
          try {
              ...
              if (enable_abtest) {
                  var that = this;
                  var userId = that.DATracker.get_user_id() || that.DATracker.get_distinct_id();
                  var appKey = that.DATracker.get_config('token');
                  var callback = function (params) {
                      that.getVariableInfoComplete = true;
                      if (!params.error) {
                          if (params.code === 200) {
                              var _default_variables = _.JSONDecode(_.JSONEncode(that._default_variables));
                              that.usedvariables = [];
                              that.data.experiments = params.data.experiments;
                              that.data.variables = _.deepMerge(_default_variables, params.data.variables);
                              that.data.updatedTime = new Date().getTime();
                              that._saveLocal();
                          }
                      }
                      if (that.queue.length) {
                          _.each(that.queue, function (t) {
                              that.get_variation.apply(that, Array.prototype.slice.call(t));
                          });
                          that.queue = [];
                      }
                  };
                  that.getVariableInfoComplete = false;
  ...
  */
  ...
  //触发
  MixpanelLib.prototype._send_request = function (url, data, callback, errorFn, otherFn) {
      if (ENQUEUE_REQUESTS) {
