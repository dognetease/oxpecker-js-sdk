...
var multilinkAbest = {
    ...
    jump: function (variableObj) {
        variableObj = variableObj || this.getVariable();
        var nowUrl = this.getNowUrl();
        if (variableObj) {
            if (variableObj.url !== nowUrl && variableObj.url !== nowUrl + '/') {
                if (this.isTestDebug()) {
    ...
    getVariable: function () {
        ...
        for (var i = 0; i < this.multilinkArr.length; i++) {
            // 去掉原key的 $符号：'$https://abtest.aa.com' ==> 'https://abtest.aa.com'
...
var abtest = {
    ...
    // 变量分类：编程实验、可视化实验、多链接实验 type  1,2,3
    ...
    _dataClass: function () {
        ...
        for (var key in variables) {
            if (variables.hasOwnProperty(key)) {
                if (this._getExperimentType(key) === 3) {
                    multilinkArr.push([key, variables[key]]);
                }
            }
        }
        multilinkAbest.setMultilinkArr(multilinkArr);
        var nowVariableObj = multilinkAbest.getVariable();
        if (nowVariableObj) {
            // 内部调用 ：执行多链接
            this.get_variation(function (abtest) {
                try {
                    var jump = function () {
                        multilinkAbest.jump(nowVariableObj);
                    };
                    //如果没有回调成功，设置超时回调
                    setTimeout(jump, abtest.abtest_config.multilinkTimeOutMs);
                    // 使用该多链接实验变量，触发da_abtest事件
                    // 由于是跳转，为了保证事件顺利发送，延迟一段时间再执行重定向。 
                    abtest.get(nowVariableObj.variable, window.location.href, jump, { variableObj: nowVariableObj });
                } catch (error) {
                    console.error(error);
                }
            });
    ...
    // 检测是否处于abtest调试模式下，该方法不仅仅在内部引入，sdk初始化时也引用
    isTestDebug: debug._isTestDebug,
    getDebugKeyData: debug.getDebugKeyData,
    _saveLocal: function () {
        var obj = {
            data: this.data
        };
        ...
        try {
            ...
            if (enable_abtest) {
                ...
                var callback = function (params) {
                    that._getVariableInfoComplete = true;
                    if (!params.error) {
                        if (params.code === 200) {
...
DATrackerLib.prototype._init = function (token, config, name) {
    ...
    heatmap.init(_self, function () {
        // 判断是否进入abtest调试，是的话，不允许上报数据给服务端
        if (DATrackerABtest.isTestDebug()) {
