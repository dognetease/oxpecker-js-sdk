...
var buffer = {
	...
	//-- ��ʼ������
	setTheme: utils.store.get('theme')||'ace/theme/chrome',
...
TinyEditor.prototype = {
	...
	init : function(el,opt){
		...
		for(var i = 0,len=buffer.tabContainer.length;i<len;i++){
			buffer.tabContainer[i].style.height= (buffer.tabPlugin.offsetHeight+ 200)+'px';
		}
	...
	dockMode : function(){
		if(!!utils.dockMode){   //-- ȡ��dockģʽ
			...
			document.getElementById('dockIframe').style.height=(buffer.tabPlugin.offsetHeight+200)+'px';
