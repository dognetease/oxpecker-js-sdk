(function (global, factory) {
  ...
  Array.prototype.contains = function (needle) {
    for (var i in this) {
      if (this.hasOwnProperty(i)) {
        if (typeof this[i].indexOf === 'function') {
  ...
  _.register_hash_event = function (callback) {
      ...
      _.register_event(window, hash_event, callback);
  };
  ...
  function app_js_bridge() {
    ...
    //android
    function getAndroid() {
      if (typeof window.HubbleData_APP_JS_Bridge === 'object' && window.HubbleData_APP_JS_Bridge.hubbledata_call_app) {
  ...
  */
  ...
  MixpanelLib.prototype._init = function (token, config, name) {
      ...
      //打通app与h5
      var getAppStatus = app_js_bridge();
  ...
  MixpanelLib.prototype._single_page = function () {
      ...
      if (this.get_config('single_page_config').mode === 'hash') {
          // _.register_hash_event(function(){ 
  ...
  MixpanelLib.prototype.track = function (event_name, properties, callback) {
      ...
      //当触发的事件是 session开始和结束时，以及是PV事件不触发 _session() 方法
      if (['da_session_start', 'da_session_close', 'da_screen'].indexOf(event_name) === -1) {
          this._session();
      }
      ...
      this._send_request(this.get_config('api_host') + '/track/w/', { 'data': encoded_data, 'appKey': appkey_data }, this._prepare_callback(callback, truncated_data));

      //最后一次触发的事件，解决
      //session_close 事件的时间计算
      if (['da_session_close', 'da_session_start'].indexOf(event_name) === -1) {
          this['cookie'].register({
              LASTEVENT: {
                  eventId: event_name,
                  time: time
   ...
   * @api private
   */
  MixpanelLib.prototype.track_pageview = function (page, call) {
      if (_.isUndefined(page)) {
          page = document.location.href;
      }
      var self = this;
      var callback = function () {
          var data = self.track('da_screen', _.info.pageviewInfo(page));
  ...
  MixpanelPeople.prototype._send_request = function (data, callback) {
      ...
      this._mixpanel._send_request(this._get_config('api_host') + '/track/w/', { 'data': encoded_data, 'appKey': appkey_data }, this._mixpanel._prepare_callback(callback, truncated_data));
